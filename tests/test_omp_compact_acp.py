"""Live ACP compact smoke test for OMP.

Requires ``omp`` on PATH and ``TAKOPI_OMP_ACP_SESSION_ID`` set to an
...
omp = pytest.importorskip("shutil").which("omp")
_SESSION_ID = os.environ.get("TAKOPI_OMP_ACP_SESSION_ID")
...
@pytest.mark.skipif(
    not _SESSION_ID,
    reason="TAKOPI_OMP_ACP_SESSION_ID not set; cannot probe live ACP",
)
@pytest.mark.anyio
async def test_omp_acp_compact_live() -> None:
    """Probe OMP ACP compact against an existing session."""
    ...
    async with AcpClient(
        ...
        try:
            ...
            pytest.skip("OMP ACP agent does not advertise 'compact'")

        updates = [u async for u in client.prompt(_SESSION_ID, f"/compact {marker}")]
        for update in updates:
            assert marker not in update.text, (
                "OMP echoed the compact marker — interception not confirmed"
            )
