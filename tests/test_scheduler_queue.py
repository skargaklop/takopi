"""Deterministic scheduler queue lifecycle and race contracts (Task 21).
...
async def test_cancel_after_claim_does_not_touch_predecessor() -> None:
    ...
    The predecessor is simulated by ``note_thread_known`` holding the thread
    busy; the only enqueued job is the queued one. After the worker claims it,
    cancelling that progress identity returns ``ALREADY_CLAIMED`` and does not
    ...
    async with anyio.create_task_group() as tg:
        scheduler = ThreadScheduler(task_group=tg, run_job=_run_job)
        # The predecessor holds the thread busy via note_thread_known.
        await scheduler.note_thread_known(resume, active_done)
        await scheduler.enqueue(_job(msg_id=10, progress_id=50, text="queued"))

        # Release the predecessor so the worker claims the queued job.
        active_done.set()
        with anyio.fail_after(5):
            await claimed_queued.wait()

        # Now the queued job is claimed (running). Cancelling must report
        # ALREADY_CLAIMED, not CANCELLED, and must not touch any other task.
        result = await scheduler.cancel_queued(123, 50)
        assert result.status is CancelQueuedStatus.ALREADY_CLAIMED
        assert result.job is None
...
async def test_enqueue_rolls_back_on_start_failure() -> None:
    ...
    # After rollback, the job must not be present in scheduler state.
    assert await scheduler.get_queued(123, 50) is None
    assert await scheduler.queue_depth(resume) == 0
    # The active marker must have been removed so a later enqueue can start.
...
async def test_enqueue_after_rollback_starts_normally() -> None:
    ...
    # Now use a real task group with the SAME thread key on a fresh scheduler.
