...
- grok answer/narration split: the final Telegram message now contains only the actual answer, not the entire reasoning transcript. In agentic multi-turn runs, `text` events between tool calls (narration) are segmented at `thought` event boundaries — earlier segments become coalesced note actions in progress, and only the trailing text run becomes the final answer. Single-turn Q&A is unchanged (full text is the answer). Fixes the 2/2-split dump of reasoning followed by the answer.
